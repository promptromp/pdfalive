...
**Evaluation Harness:**
The `pdfalive/evaluation/` package measures TOC generation quality against golden (ground truth) data. A case is fully defined by `evals/golden/<name>.json` (source PDF path resolved relative to the evals dir's parent, a `postprocess` flag, and the expected entries). `metrics.py` contains pure scoring functions: golden and generated entries are greedily matched by normalized-title similarity (threshold 0.8, page distance as tiebreaker for repeated titles like "Exercises"). Matching is tolerant of dropped section numbering: a pair that agrees only after `strip_section_prefix()` removes leading numbering ("Chapter 1: Distance and Angles" vs "Distance and Angles") still matches, scored at 0.95x so exact full-title matches always pair first — without this, one model's choice to omit a printed chapter number is charged twice, as both a missing golden entry and a spurious generated one. Scoring yields precision/recall/F1, page accuracy (exact and within ±N pages), and level accuracy. `cassette.py` provides VCR-style record/replay: `RecordingChatModel` wraps a real LLM and persists each structured response (with a SHA-256 request fingerprint) to `evals/cassettes/<name>.json` after every call; `ReplayChatModel` serves them back in order with zero network calls. Both duck-type the `with_structured_output(schema).invoke(messages)` interface that processors use. Strict replay (default) fails loudly on prompt drift — re-record the cassette when prompts/features change intentionally. Cassettes also store each call's `usage_metadata`, so replays report the recorded run's real token usage. The eval runner does not perform OCR: scanned books are pre-OCR'd once (`pdfalive extract-text`) into local copies (e.g. `data/ocr/`, gitignored) and the golden's `source_pdf` points at the OCR'd copy. The golden TOCs for `geometry` and `arnold` are human-verified (arnold: pipeline output corrected against page text). Arnold intentionally scores <1.00 until known pipeline defects are fixed. Benchmarking across models and repeated samples showed three that reproduce in every run: 'Appendix 11 Short wave asymptotics' (golden p455, pipeline p453), 'Appendix 14 Poisson structures' (golden p473, pipeline p477), and a spurious level-1 'Bibliography...' entry at p520 that the golden lacks — that last one looks like a real heading the golden omits rather than a pipeline error. The same-title chapter/subsection pair 'Chapter 7 Differential forms' vs '§34 Differential forms' is the case `strip_section_prefix()` must not collapse, and is covered by a metrics test. The `mitrinovic` golden was seeded from prior bookmarks, which the current pipeline reproduces exactly. Cassettes are recorded with the default model (`gpt-5.6`). Note that a cassette captures one draw from a high-variance distribution — first-pass entry counts vary substantially between runs with everything else held fixed — so the replay baseline is a fixed reference for testing post-LLM logic, not a stable measure of model quality. In particular the recorded `arnold` draw is a two-level one, so its level accuracy reads 0.28 against the golden's three-level hierarchy; see the depth-cap mismatch noted above.
